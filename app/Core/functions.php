...
/**
 * Encode an ID for URLs to hide the underlying database integer
 */
function encodeProductId($id) {
    if (!$id) return null;
    $salt = 98765;
    return bin2hex(base64_encode('UG' . ($id * $salt)));
}

/**
 * Decode an encoded ID from URLs
 */
function decodeProductId($encoded) {
    if (!$encoded || !ctype_xdigit($encoded) || strlen($encoded) % 2 !== 0) return null;
    $decoded = base64_decode(hex2bin($encoded));
    $salt = 98765;
    if (strpos($decoded, 'UG') === 0) {
        $numStr = substr($decoded, 2);
        if (is_numeric($numStr)) {
            return (int)($numStr / $salt);
        }
    }
    return null;
}
