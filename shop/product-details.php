...
$encoded_id = $_GET['id'] ?? '';
$product_id = decodeProductId($encoded_id);
if (!$product_id) {
...
<div class="bg-[#eef2f9] min-h-[calc(100vh-80px)] py-10">
            ...
            <div class="flex flex-col lg:flex-row gap-12 lg:gap-20 pb-10">
                ...
                <div class="lg:w-1/2 flex flex-col items-center">
                    ...
                    <div class="bg-white rounded-[24px] flex items-center justify-center p-10 h-[500px] w-full relative mb-8">
                        ...
                        <img src="<?= SITE_URL ?>/assets/img/<?= $product['image'] ?>" alt="<?= sanitize($product['name']) ?>" class="max-h-[420px] object-contain drop-shadow-2xl" id="mainImage" onerror="this.src='https://via.placeholder.com/400x300?text=<?= urlencode($product['name']) ?>'">
                    ...
                    <div class="flex gap-4 mb-8 overflow-x-auto pb-2 justify-center w-full">
                        ...
                        <div class="w-[84px] h-[84px] flex-shrink-0 rounded-[14px] bg-black border-2 <?= $index === 0 ? 'border-[#3b5ae6]' : 'border-transparent hover:border-gray-200' ?> cursor-pointer transition-all overflow-hidden relative thumbnail-btn" 
                             onclick="document.getElementById('mainImage').src='<?= SITE_URL ?>/assets/img/<?= $img['image_url'] ?>'; document.querySelectorAll('.thumbnail-btn').forEach(b => { b.classList.remove('border-[#3b5ae6]'); b.classList.add('border-transparent'); }); this.classList.remove('border-transparent'); this.classList.add('border-[#3b5ae6]');">
                            <img src="<?= SITE_URL ?>/assets/img/<?= $img['image_url'] ?>" alt="" class="w-full h-full object-cover opacity-90 hover:opacity-100 transition-opacity">
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <?php endif; ?>
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="w-8 h-8 bg-[#111] rounded-full flex items-center justify-center p-1.5 overflow-hidden">
                                     <img src="<?= SITE_URL ?>/assets/img/<?= $product['brand_logo'] ?>" class="w-full h-full object-contain filter invert bg-white">
                                </div>
                                <?php endif; ?>
    ...
    <section class="max-w-[1200px] mx-auto px-6 pb-10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <a href="product-details.php?id=<?= encodeProductId($rp['id']) ?>" class="bg-white rounded-[20px] p-6 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300">
                <div class="h-44 bg-transparent flex items-center justify-center mb-5">
                    <img src="<?= SITE_URL ?>/assets/img/<?= $rp['image'] ?>" alt="<?= sanitize($rp['name']) ?>" class="max-h-full object-contain" onerror="this.src='https://via.placeholder.com/200?text=<?= urlencode($rp['name']) ?>'">
                </div>
                <div>
