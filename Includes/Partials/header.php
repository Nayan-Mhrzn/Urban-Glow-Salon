    ...
    <nav class="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200" id="mainNav">
        <div class="max-w-7xl mx-auto px-6 h-[88px] flex items-center justify-between gap-6">
            ...
            <div class="flex-1 max-w-md hidden md:block">
                <form action="<?= SITE_URL ?>/<?= $currentPage === 'services' ? 'booking/services' : 'shop/products' ?>.php" method="GET" class="flex items-center bg-gray-100 rounded-full px-5 py-2 border-2 border-transparent focus-within:border-primary focus-within:bg-white focus-within:shadow-[0_0_0_4px_rgba(67,57,242,0.1)] transition-all">
                    ...
                    <div class="relative" id="userDropdown">
                        <button class="flex items-center gap-2 px-4 py-2.5 rounded-full text-[16px] font-bold text-gray-700 hover:bg-[#EEF0FF] hover:text-[#4f46e5] transition-all" id="userTrigger">
                            <?php if (isset($_SESSION['profile_image']) && !empty($_SESSION['profile_image'])): ?>
                                <img src="<?= SITE_URL ?>/assets/uploads/profiles/<?= sanitize($_SESSION['profile_image']) ?>" alt="Profile" class="w-7 h-7 rounded-full object-cover border border-gray-200">
                            <?php else: ?>
