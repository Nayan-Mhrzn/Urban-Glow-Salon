...
$uploadDir = SITE_ROOT . '/assets/uploads/products/';
            ...
            foreach ($_FILES['images']['name'] as $i => $name) {
                if ($_FILES['images']['error'][$i] === 0 && in_array($_FILES['images']['type'][$i], $allowed)) {
                    ...
                    if (move_uploaded_file($_FILES['images']['tmp_name'][$i], $uploadDir . $fileName)) {
                        $sortOrder++;
                        $insertStmt->execute([$productId, 'assets/uploads/products/' . $fileName, $sortOrder]);
                    }
                }
            }
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-24">
            ...
            <div class="mt-6 pt-6 border-t border-gray-100">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <img src="<?= SITE_URL ?>/<?= $product['image'] ?>" alt="" class="w-12 h-12 rounded-xl object-cover border border-gray-100" onerror="this.src='<?= SITE_URL ?>/assets/img/<?= $product['image'] ?>'">
                    <?php endif; ?>
                    <div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div id="sortableGrid" class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div class="sortable-item relative group rounded-2xl overflow-hidden border-2 border-gray-100 hover:border-primary transition-all cursor-grab active:cursor-grabbing" data-id="<?= $img['id'] ?>">
                    <img src="<?= SITE_URL ?>/<?= $img['image_url'] ?>" alt="Product image" class="w-full aspect-square object-cover" onerror="this.src='<?= SITE_URL ?>/assets/img/<?= $img['image_url'] ?>'">
